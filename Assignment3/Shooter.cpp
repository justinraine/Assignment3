...
void shooterAction(int rateShotsPerSecond, Color playerColor) {
    ...
    while (execute) {
        ...
        // Only check if all lanes are full if there is reason to think they could be
        if (roundShotCount - violetLanes >= lanesGallery->Count()/2) {
            // Check to see if all lanes are full. If so, reset non-violet lanes back to white
        }
